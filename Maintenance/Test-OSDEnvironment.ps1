...
$osIs24H2  = $osCaption -match 'Windows 11' -and $osBuild -eq '26100'
$osResult  = if ($osIs24H2) { 'PASS' } else { 'WARN' }
$osDetail  = if ($osIs24H2) { "$osCaption (build $osBuild)" } else { "$osCaption (build $osBuild) - Windows 11 24H2 recommended for 24H2 WinRE" }
Add-Check 'Build Machine OS (Win11 24H2)' $osResult $osDetail
...
if (Test-Path $startnet) {
    ...
    $expectedUrl = 'https://codostpublicassets.blob.core.windows.net/osdcloud/Deploy-Windows11.ps1'
