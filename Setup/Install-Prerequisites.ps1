...
.DESCRIPTION
    ...
    Run on a Windows 11 24H2 build machine to produce a Windows 11 24H2 WinRE boot image.
    Target hardware must support TPM 2.0 and Secure Boot for Windows 11 WinRE.
...
.NOTES
    ...
    Requires: Windows 11 24H2 build machine, internet access
#>
...
if ($adkOk) {
    ...
    if ($adkBinary) {
        ...
        # Windows 11 24H2 ADK build is 26100. Accept ADK build matching the OS build.
        $win11_24H2Build = '26100'
        $adkVersionMatch = $adkBuild -eq $win11_24H2Build -and $osBuild -eq $win11_24H2Build
        $adkVersionDetail = if ($adkVersionMatch) { "ADK build $adkBuild matches OS build $osBuild (Win11 24H2)" } else { "OS build $osBuild vs ADK build $adkBuild - MISMATCH (need Win11 24H2 ADK, build 26100)" }
    } else {
...
if (-not $adkOk) {
    ...
    Write-Host ''
    Write-Host '  Required: Windows ADK for Windows 11, version 24H2 (build 26100)' -ForegroundColor White
    Write-Host ''
    ...
    Write-Host '  2. Download the latest ADK (Windows 11, version 24H2)' -ForegroundColor White
    Write-Host '  3. Run adksetup.exe and select Deployment Tools only' -ForegroundColor White
    Write-Host ''
...
if ($adkOk -and -not $adkVersionMatch) {
    ...
    Write-Host ''
    Write-Host '  Required: Windows ADK for Windows 11, version 24H2 (build 26100)' -ForegroundColor White
    Write-Host '  A mismatched ADK causes 0x800f081e errors when building the WinPE template.' -ForegroundColor White
    Write-Host ''
    ...
    Write-Host '     https://learn.microsoft.com/en-us/windows-hardware/get-started/adk-install' -ForegroundColor Cyan
    Write-Host '  3. Download the latest ADK (Windows 11, version 24H2)' -ForegroundColor White
    Write-Host '  4. Run adksetup.exe and select Deployment Tools only' -ForegroundColor White
    Write-Host ''
...
if (-not $winPEOk) {
    ...
    Write-Host '  Required: WinPE Add-on for Windows 11, version 24H2' -ForegroundColor White
    Write-Host ''
    Write-Host '  1. Go to the ADK download page:' -ForegroundColor White
    Write-Host '     https://learn.microsoft.com/en-us/windows-hardware/get-started/adk-install' -ForegroundColor Cyan
    Write-Host '  2. Download the latest WinPE Add-on (Windows 11, version 24H2)' -ForegroundColor White
    Write-Host '  3. Run adkwinpesetup.exe and select Windows Preinstallation Environment' -ForegroundColor White
    Write-Host ''
    Write-Status 'Re-run this script after installing.' -Status 'WARN'
