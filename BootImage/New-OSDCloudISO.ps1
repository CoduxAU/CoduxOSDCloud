...
.DESCRIPTION
    Creates two ISO variants from the active workspace:
      - Standard:   CoduxOSDCloud.iso  - prompts for a keypress on boot before starting WinPE.
      - NoPrompt:   CoduxOSDCloud-NoPrompt.iso - boots directly into WinPE without any keypress prompt.

    Upload CoduxOSDCloud.iso to:
...
foreach ($iso in $generatedISOs) {
    ...
    if ($iso.Name -like '*NoPrompt*') {
        $dest = Join-Path $OutputPath 'CoduxOSDCloud-NoPrompt.iso'
        ...
        $dest = Join-Path $OutputPath 'CoduxOSDCloud.iso'
        Move-Item -Path $iso.FullName -Destination $dest -Force
        $isoStandard = $dest
